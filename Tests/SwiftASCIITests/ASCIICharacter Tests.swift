...
@Suite
struct ASCIICharacterTests {
    @Test("init(exactly: Character)")
    func init_exactly_Character() {
    ...
    @Test("init(exactly: StringProtocol)")
    func init_exactly_StringProtocol() {
        #expect(ASCIICharacter(exactly: "A string") == nil)
    }

    @Test("init(BinaryInteger)")
    func init_exactly_Data() {
    ...
    func customStringConvertible() throws {
        #expect(try String(describing: #require(ASCIICharacter(65))) == "A")

        // non-printable ASCII chars
        #expect(try String(describing: #require(ASCIICharacter(0))) == "?")
    }
    ...
    func staticConstructors() throws {
        ...
        let _: [ASCIICharacter] = try [
            .lossy("A"),
            .lossy("A string"),
            .lossy(Character("A")),
            #require(.exactly(Character("A"))),
            #require(.exactly("A")),
